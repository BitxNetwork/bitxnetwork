// CONSTANTS
var contractAddress = '0xa258850d9c6983d0d5ef4c90f2896395131ee9dc';
...
function updateData () {
  ...
  if (currentAddress !== null) {
    ...
    $('#eth-address').html('Please log into Metamask!')
  }
	...
	$('#purchase-amount').on('input change', function() {
        ...
        if(value_ether) {
            ...
            contract.calculateTokensReceived(value_wei, function (e, r) {
                $('#deposit-hint').text("You will get approximately " + r.div(1e18).toFixed(0) + " BITX");
            });
        } else {
            $('#deposit-hint').text("");
        }
...
if (walletMode === 'metamask') {
   contract.myTokens(function (err, myTokens) {
       if (parseFloat(amount) <= parseFloat(myTokens)) {
           contract.transfer(address, amount, function (err, result) {
               if (err) {
                    alertify.error('An error occured. WTF!');
   ...
   alertify.error('Transfer functionality supported only with Metamask or Trust Wallet.');
}}  
